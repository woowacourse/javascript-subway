...
- [ ] 회원 가입 시 `email`, `name`, `password`를 제출해야한다.
  - [ ] 회원 가입 시 입력받는 email은 ...@... 형식을 가진다.
  - [ ] 회원 가입 시 name은 알파벳, 한글, 숫자형식을 가진다.
  - [ ] 회원 가입 시 비밀번호와 비밀번호 확인 문자가 같아야 된다.
### 로그인 페이지
- [ ] 알맞은 이메일과 비밀번호를 입력해야 로그인할 수 있다.
- [ ] 로그인 하게되면 로그인 버튼은 로그아웃 버튼으로 변경된다.
- [ ] 로그인하지 않은 유저에게는 로그인 외 버튼은 보이지 않는다.
- [ ] 로그아웃하고 나면 로그인 버튼으로 변경되어야 한다.
### 정보수정 페이지
- [ ] 로그인 하게되면 정보 수정 페이지로 접근이 가능하다.

### ux/ui
- [ ] 회원 가입 input에 잘못된 값이 들어오면 ui상으로 보여준다.
