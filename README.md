...
- [ ] 유저가 노선 추가 버튼을 누른다.
  - [ ] 지하철 노선은 2자 이상 10자 이하여야 한다.
  - [ ] 중복된 지하철 노선 이름은 추가할 수 없다.
  - [ ] 색상, 상행역, 하행역, (최초 상행역과 하행역 구간의)거리, 시간을 입력한다.
- [ ] 유저가 확인 버튼을 누른다.
  - [ ] 엔터키 또는 마우스 클릭으로 노선을 추가할 수 있어야 한다.
- [ ] 추가된 노선이 화면에 보인다.

**수정**

- [ ] 유저가 지하철 노선 이름 옆 수정 버튼을 누른다.
  - [ ] 지하철 노선의 이름, 상행역, 하행역, 거리, 시간, 색상을 수정할 수 있다.

**삭제**

- [ ] 유저는 지하철 노선 이름 옆 삭제 버튼을 누른다.
  - [ ] 삭제 시 confirm을 이용하여 한번 더 유저가 확인할 수 있어야 한다.
...
- [ ] 유저가 구간 추가 버튼을 누른다.
  ...
  - [ ] 구간 추가 모달이 뜬다.
  - [ ] 상행역, 하행역이 모두 노선 안에 포함되어 있으면 추가할 수 없다.
  - [ ] 역과 역 사이에 새로운 역이 추가될 수 있다.
  - [ ] 노선에서 갈래길은 생길 수 없다.

- [ ] 유저가 확인 버튼을 누른다.
  - [ ] 엔터키 또는 마우스 클릭으로 구간을 추가할 수 있어야 한다.
- [ ] 추가된 구간이 화면에 보인다.

**삭제**

- [ ] 유저가 역 이름 옆에 삭제 버튼을 누른다.
  - [ ] 노선에서 역이 삭제된다.
