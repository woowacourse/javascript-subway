...
- [x] 회원가입 요청 시 검증을 해야한다.
  - [x] 중복된 이메일로 가입 시 alert 메시지를 띄운다.
  - [x] 비밀번호 입력란 값과 비밀번호 확인 입력란 값이 다를 경우 alert 메시지를 띄운다.
  - [x] 이름 입력란에 한글, 영어 외의 형식 입력 시 alert 메시지를 띄운다.
  - [x] 비밀번호를 8자 미만으로 입력 시 alert 메시지를 띄운다.
- [x] 유저는 로그인 할 수 있다.
