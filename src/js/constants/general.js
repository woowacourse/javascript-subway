export const TOKEN_STORAGE_KEY = "token";

export const PASSWORD_MIN_LENGTH = 6;
export const STATION_NAME_MIN_LENGTH = 2;
export const STATION_NAME_MAX_LENGTH = 20;
export const LINE_NAME_MIN_LENGTH = 2;
export const LINE_NAME_MAX_LENGTH = 10;
export const LINE_MIN_DISTANCE = 1;
export const LINE_MIN_DURATION = 1;
export const STATION_LIST_ITEM_BORDER_HEIGHT = 2;

export const EMAIL_REG_EXP = /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]{2,3}$/i;
export const SPACE_REG_EXP = /[ \t\r\n\v\f]/gi;

export const LINE_MODAL_MODE = {
  ADD: "ADD",
  MODIFY: "MODIFY",
};
